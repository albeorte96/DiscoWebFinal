...
<center>
	...
	switch ($plan){
	    case "Basico":
	        echo ($espacio/1000)."/"."2000 KB (".round(((($espacio/1000)/2000)*100),2,null)."%)";
	        break;
	    case "Profesional":
	        echo ($espacio/1000)."/"."5000 KB (".round(((($espacio/1000)/5000)*100),2,null)."%)";
	        break;
	    case "Premium":
	        echo ($espacio/1000)."/"."10000 KB (".round(((($espacio/1000)/10000)*100),2,null)."%)";
	        break;
	}
...
<?php foreach ($ficheros as $clave => $datosusuario) : ?>
		...
		<tr>
			...
			<td class="renombrar"><a href="#" onclick="confirmarRenombrarfichero('<?= $datosusuario[0]."','".$clave."'"?>);">&#9998;</a></td>
			<td class="descarga"><a href="<?= $auto?>?operacion=Descargar&id=<?= $datosusuario[0]?>">Descargar</a></td>
		</tr>
<?php endforeach; ?>
</table>
