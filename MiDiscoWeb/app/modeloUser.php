...
function calcular_ficheros($userid){
    $suma=0;
        ...
        if ($usuario == $userid) { // si el usuario coincide con el usuario de la sesion
            ...
            foreach ($_SESSION['ficheros'][$userid] as $nombrefich => $datos) { // realizo un for each para recorrer la tabla de archivos con sus respectivos datos.
               // echo "<br>" . $nombrefich;
                ...
                $suma++;//sumo los archivos
            }
        }
    }
    echo "FICHERO TOTALES: ".$suma;
    return $suma;
}
...
function cumpleEspacio($user, $plan, $tamanho){
    ...
    switch ($plan) {
        case "Basico":
            ...
            if(($tamanho + $espacio)< 2000000 && calcular_ficheros($user)<LIMITE_FICHEROS[0]) return true;
            break;
        case "Profesional":
            //Asignamos 5MB de espacio para el plan Profesional
            if(($tamanho + $espacio)< 5000000 && calcular_ficheros($user)<LIMITE_FICHEROS[1] ) return true;
            break;
        case "Premium":
            //Asignamos 10MB de espacio para el plan Premium
            if(($tamanho + $espacio)< 10000000 && calcular_ficheros($user)<LIMITE_FICHEROS[2] ) return true;
            break;
    }
    return false;
}
